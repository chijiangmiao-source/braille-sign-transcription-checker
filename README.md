...
只输入门牌文本即可核算本次压印用量，统计由同一编码结果推导：单元总数为编码单元个数，凸点总数为各单元点数之和（空点计 0），空点数为 `0` 单元个数，数字号数为 `3456` 单元个数。文本被修改或再次核算时旧用量即被清除，核算响应返回时文本已修改的，过期结果直接丢弃、保持清空；核算失败只在用量区域提示，不影响已有核对结论。
